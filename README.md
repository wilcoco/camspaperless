...
구성원에게 **업무(점검·체크리스트)** 를 할당하고, **주기별(일/주/격주/월/분기/반기/년/N일, 주기당 N회)** 로
수행 여부를 확인·리포트·경고하는 웹앱입니다. 모바일 우선 반응형 UI로 현장에서 **사진·점검일자·특이사항**을 입력하고,
...
### 관리자
1. **업무관리** 에서 업무 등록 — 주기는 **프리셋 칩**(매일/매주/주2회/격주/매월/분기/반기/매년)으로 선택하거나
   **직접 설정**(매 N주·월 + 주기당 N회 + N일마다)으로 지정. 폼 아래 **미리보기**로 이번/다음 주기 범위를 즉시 확인.
   체크리스트는 **항목별로 증빙 조합**(체크·텍스트·사진·위치·QR을 한 항목에 복수 지정)을 선택
...
- 각 업무는 자체 주기를 가지며, 수행 기록은 해당 주기 키(예: `2026-06`, `2026-W23`, `2026-Q2`, `C5`)에 귀속됩니다.
  격주·반기 등 **간격 주기**(매 N주/월)는 업무 시작일을 앵커로 한 키(`W2N0`, `M6N3`)를 씁니다.
- **주기당 수행 횟수**(`times_per_period`)가 지정된 업무는 그 횟수를 채워야 `완료`로 판정됩니다 (예: 매주 2회).
- 상태: `완료`(현재 주기 요구 횟수 충족) · `대기` · `마감임박`(마감 N일 전) · `미수행`(의무였던 직전 주기 누락).
